...
pub struct LogPanel<'a, T: LogItem> {
    ...
    /// Whether the marked items are the ones the next action takes,
    /// which the panel says in its title
    pub use_marks: bool,
...
impl<T: LogItem> Component for LogPanel<'_, T> {
    ...
    fn draw(&mut self, f: &mut Frame<'_>, area: Rect) -> Result<()> {
        ...
        let mut log_block = panel_block().title(panel_title(self.title.clone()));
        if self.use_marks && !self.marked.is_empty() {
            log_block = log_block.title_top(
                Line::styled(" marked ", Role::Warning.style())
                    .bold()
                    .right_aligned(),
            );
        }
