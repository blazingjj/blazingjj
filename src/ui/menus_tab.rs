            ...
            .map(|(index, row)| {
                let line = match row {
                    ...
                    SectionRow::Item(listed) => {
                        ...
                        Line::from(vec![
                            Span::raw(format!("{INDENT}{place}")).patch_style(Role::Hint.style()),
                            // An item the menu does not hold is one that
                            // is not there to be picked, whatever it is.
                            if listed.place.is_some() {
                                id
                            } else {
                                id.patch_style(Role::Hint.style())
                            },
                            Span::raw(what).patch_style(Role::Hint.style()),
                        ])
                    }
                };
...
impl Component for MenusTab {
    fn draw(&mut self, f: &mut Frame<'_>, area: Rect) -> Result<()> {
        ...
        let block = panel_block()
            .title(panel_title(" Settings / Context menus "))
            .title_bottom(
