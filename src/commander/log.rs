...
impl Commander {
    fn execute_jj_log(
        ...
        let mut args = vec!["log", "--no-graph", "--template", template, "-r", revset];
        if limit_one {
            args.extend(["--limit", "1"]);
        }
        self.execute_jj_command(args, false, true)
    }
    ...
    pub fn get_current_head(&self) -> Result<Head> {
        parse_head(
            &self
                .execute_jj_log("@", HEAD_TEMPLATE_NL, true)
    ...
    pub fn get_commit_parent(&self, commit_id: &CommitId) -> Result<Head> {
        parse_head(
            &self
                .execute_jj_log(&format!("{commit_id}-"), HEAD_TEMPLATE_NL, true)
    ...
    pub fn get_commit_description(&self, commit_id: &CommitId) -> Result<String> {
        Ok(self
            .execute_jj_log(commit_id.as_str(), "description", true)
    ...
    pub fn check_revision_immutable(&self, revision: &str) -> Result<bool> {
        Ok(self
            .execute_jj_log(revision, "immutable", true)
    ...
    pub fn get_bookmark_head(&self, bookmark: &Bookmark) -> Result<Head> {
        parse_head(
            &self
                .execute_jj_log(&bookmark.to_string(), HEAD_TEMPLATE_NL, true)
