...
impl CustomCommand {
    ...
    /// the arguments filled in from it, and whether it names what the
    /// log has marked.
    pub fn program(&self, selection: &Selection) -> Result<(Program, bool), Missing> {
        let env = get_env();
        let substituted = selection.substitute(&self.args)?;

        Ok((
            Program::new(&self.program, env.root.clone()).args(substituted.args),
            substituted.uses_marks,
        ))
    }
...
mod tests {
    ...
    /// A command that names the marks is what the marks were there for,
    /// so running it is what they are done being needed for.
    #[test]
    fn a_command_says_whether_it_names_the_marks() {
        set_test_env();
        let marked = Selection::default().marked(&[CommitId("commit".to_owned())]);

        let (_, uses_marks) = command("blazingjj.commands.log = [\"jj\", \"log\"]\n", "log")
            .program(&marked)
            .expect("the command names no placeholder to fill in");
        assert!(!uses_marks);

        let (_, uses_marks) = command(
            "blazingjj.commands.diff = [\"jj\", \"diff\", \"--revisions=$marked\"]\n",
            "diff",
        )
        .program(&marked)
        .expect("the selection fills the placeholder in");
        assert!(uses_marks);
    }
}
